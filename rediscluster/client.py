...
class StrictRedisCluster(StrictRedis):
    ...
    def cluster_try_lock(self, lock_id, lock_valid_time, clock_drift):
        """
        :param lock_id:
        :param lock_valid_time:
        :param clock_drift:
        :return:
        """
        unique_id = str(uuid.uuid1())
        begin_lock_time = int(time.time())
        get_lock_count = 0
        lock_keys = []
        for master_node in self.connection_pool.nodes.all_masters():
            min_slots = [slot[0] for slot in master_node["slots"]]
            lock_keys.append("%s{%s}" % (lock_id, crckey[min(min_slots)]))
        for lock_key in lock_keys:
            if self.execute_command("SET", lock_key, unique_id, "NX", "PX", lock_valid_time):
                get_lock_count += 1
        elapsed_time = int(time.time()) - begin_lock_time
        new_begin_lock_time = int(time.time())
        new_lock_valid_time = lock_valid_time - elapsed_time - clock_drift
        if new_lock_valid_time > 0 and get_lock_count > len(lock_keys) / 2:
            is_get_lock = True
        else:
            is_get_lock = False
        return DistLock(unique_id, new_lock_valid_time, new_begin_lock_time, is_get_lock, lock_keys)

    def cluster_release_lock(self, distlock):
        """
        :param distlock:
        :return:
        """
        distlock.is_get_lock = False
        for lock_key in distlock.lock_keys:
            if self.get(lock_key) == distlock.unique_id:
                self.delete(lock_key)

    def acquire_fair_semaphore(self, semaphore_name, resource, timeout=10):
        if not self.semaphore_client:
            min_slot = 1000000
            for master_node in self.connection_pool.nodes.all_masters():
                min_slots = [slot[0] for slot in master_node["slots"]]
                if min_slot > min(min_slots):
                    min_slot = min(min_slots)
                    min_slot_host, min_slot_port = master_node["host"], master_node["port"]
            self.semaphore_tag = "{%s}" % crckey[min_slot]
            self.semaphore_client = redis.Redis(host=min_slot_host, port=min_slot_port, db=0)
        semaphore_name += self.semaphore_tag
        semaphore_id = str(uuid.uuid4())
        czset = semaphore_name + ':owner'
        ctr = semaphore_name + ':counter'
        now = time.time()
        pipeline = self.semaphore_client.pipeline(True)
        pipeline.zremrangebyscore(semaphore_name, '-inf', now - timeout)
        pipeline.zinterstore(czset, {czset: 1, semaphore_name: 0})

        pipeline.incr(ctr)
        counter = pipeline.execute()[-1]

        pipeline.zadd(semaphore_name, semaphore_id, now)
        pipeline.zadd(czset, semaphore_id, counter)

        pipeline.zrank(czset, semaphore_id)
        if pipeline.execute()[-1] < resource:
            return semaphore_id
        pipeline.zrem(semaphore_name, semaphore_id)
        pipeline.zrem(czset, semaphore_id)
        pipeline.execute()
        return None

    def release_fair_semaphore(self, semaphore_name, semaphore_id):
        semaphore_name += self.semaphore_tag
        pipeline = self.semaphore_client.pipeline(True)
        pipeline.zrem(semaphore_name, semaphore_id)
        pipeline.zrem(semaphore_name + ':owner', semaphore_id)
