    ...
    def initialize(self):
        ...
        for node in nodes:
            ...
            for slot in cluster_slots:
                ...
                slot_list = node_slots_cache.setdefault(node_name, [])
                slot_list.append([slot[0], slot[1]])

                for i in range(int(slot[0]), int(slot[1]) + 1):
                    # i is one hash slot in node
        ...
        for node_name, slots in node_slots_cache.iteritems():
            self.nodes[node_name]["slots"] = slots
        self.reinitialize_counter = 0
